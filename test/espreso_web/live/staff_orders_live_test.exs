defmodule EspresoWeb.StaffOrdersLiveTest do
  ...
  test "new unpaid ticket shows Cash GCash Maya; Prepare waits for payment", %{
    ...
    assert has_element?(view, "#{detail_id(order.id)} .staff-order-fulfillment", "Take Out")
    ...
    {:ok, order} =
      Orders.create_order(
        [
          ...
          %{name: "Muffin", size: nil, category: "FOOD", quantity: 1, price: Decimal.new("85")}
        ],
        %{customer_name: "QR Guest", fulfillment: :pickup, payment_method: :counter}
      )

    {:ok, view, _html} = live(conn, ~p"/orders")
    items = "#{detail_id(order.id)} .staff-order-items"

    assert has_element?(view, items, "Americano")
    assert has_element?(view, "#{items} .staff-order-item-temp.is-hot", "Hot")
    assert has_element?(view, "#{items} .staff-order-item-temp.is-iced", "Iced")
    refute has_element?(view, "#{items} .staff-order-item-temp", "FOOD")
  end
  ...
  test "Cash opens tender modal and exact confirmation moves the paid order to Preparing", %{
    ...
    assert has_element?(view, "#order-ready-#{order.id}.staff-action-primary", "Mark Ready")
  ...
  test "Ready ticket shows items and Complete & Archive as primary action", %{conn: conn} do
    {:ok, order} =
      Orders.create_order(
    ...
    assert has_element?(view, "#orders-preparing #order-card-preparing-#{order.id}")
    view |> element("#order-ready-#{order.id}", "Mark Ready") |> render_click()
    ...
    assert has_element?(view, "#{detail_id(order.id, "ready")} .staff-order-pay", "Paid · Cash")

    assert has_element?(
             view,
             "#ready-complete-#{order.id}.staff-action-primary",
             "Complete & Archive"
           )
  ...
  test "online paid ticket can ready and be picked up", %{conn: conn} do
    ...
    assert has_element?(view, "#order-ready-#{order.id}", "Mark Ready")
    refute has_element?(view, "#order-prepare-#{order.id}")
    refute has_element?(view, "#order-card-new-#{order.id} button", "Cash")
    ...
    assert has_element?(view, "#ready-complete-#{order.id}", "Complete & Archive")

    view |> element("#ready-complete-#{order.id}") |> render_click()
    assert has_element?(view, "#orders-flash", "Picked up · #{order.number}")
  ...
  test "kitchen tickets show order age from inserted_at across lanes", %{conn: conn} do
    ...
    assert has_element?(view, "#order-ready-#{preparing.id}.staff-action-primary", "Mark Ready")

    assert has_element?(
             view,
             "#ready-complete-#{ready.id}.staff-action-primary",
             "Complete & Archive"
           )
  ...
  test "customer source shows QR badge; pos source shows Walk-in badge", %{conn: conn} do
    ...
    assert has_element?(
             view,
             "#order-card-new-#{qr_order.id} .staff-order-ticket-head-side .staff-order-fulfillment",
             "Take Out"
           )
  ...
  test "order notes render in a prominent NOTE block before primary action", %{conn: conn} do
    ...
    assert has_element?(view, "#{detail_id(order.id)} .staff-order-fulfillment", "Dine In")
  ...
  test "Ready lane keeps more than 10 ready orders visible", %{conn: conn} do
    ...
    assert has_element?(view, "#ready-complete-#{oldest.id}", "Complete & Archive")
  ...
  test "qrph awaiting_payment order shows Waiting and staff can mark paid with GCash", %{
    conn: conn
  } do
    ...
    assert has_element?(view, "#{detail_id(order.id)} .staff-order-pay", "Waiting")
  ...
  test "new order alert still shows on Orders without a header bell", %{conn: conn} do
    ...
    assert has_element?(view, "#orders-toasts #orders-alert-banner", order.number)
