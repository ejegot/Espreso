defmodule EspresoWeb.AttendanceExportController do
  @moduledoc """
  Authenticated attendance Excel download for Owner/Manager.
  """
  use EspresoWeb, :controller

  alias Espreso.Accounts.Authorization
  alias Espreso.Orders
  alias Espreso.Reports.AttendanceExport
  ...
  def download(conn, params) do
    ...
    with true <- Authorization.can?(user, :reports),
         ...
         {:ok, pack} <- AttendanceExport.pack(from_date, to_date),
         {:ok, {filename, binary}} <- AttendanceExport.build_xlsx(pack) do
      conn
      ...
      {:error, reason} ->
        fail(conn, "Unable to export attendance (#{inspect(reason)}). Try again.")
    end
  end
